...
| `LLM_REQUIREMENT_IMAGES_ENABLED`      | `true`                     | When `true`, the UI can attach PNG/JPEG/GIF/WebP images and PDF mockups (uploads and selected JIRA ticket attachments). Images are sent as vision inputs; PDFs are sent as chat `file` parts (OpenAI-style). Requires a model/server that supports those modalities.         |
